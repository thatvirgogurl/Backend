...
const UpdateCart = async function(req,res){
    try{
        ...
        if(!mongoose.isValidObjectId(productId)) return res.status(400).send({status:false,message:"please provied a valid productId"}) 
        if(!mongoose.isValidObjectId(cartId)) return res.status(400).send({status:false,message:"please provied a valid cart Id"})

        let product = await productModel.findOne({_id:productId,isDeleted:false})
        if(!product) return res.status(400).send({status:false,message:"product is not exist"})
 
        let cartExist = await cartModel.findOne({userId:userId,_id:cartId})
        if(!cartExist) return res.status(400).send({status:false,message:"cart is not exist"})
         
            
            
       let incProductQuantity = await cartModel.findOneAndUpdate({"items.productId":productId}
                ,{$inc:{totalPrice:product.price,"items.$.quantity":1}},{new:true})
         res.status(201).send({status:true,message:"product quantity increased",data:incProductQuantity})
